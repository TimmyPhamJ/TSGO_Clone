...
(function(e) {
	"function" === typeof define && define.amd ? define(["jquery", "datatables.net"], function(m) {
		return e(m, window, document)
	}) : "object" === typeof exports ? module.exports = function(m, k) {
		m || (m = window);
		if (!k || !k.fn.dataTable) k = require("datatables.net")(m, k).$;
		return e(k, m, m.document)
	...
	e.extend(n.prototype, {
		...
		_constructor: function() {
			...
			e(b.table().body()).on("click.keyTable", "th, td", function(c) {
				if (!1 !== a.s.enable) {
					var d = b.cell(this);
					d.any() && a._focus(d, null, !1, c)
				}
			});
			e(k).on("keydown.keyTable", function(b) {
				a._key(b)
			});
			...
			if (this.c.editor) b.on("key.keyTable", function(b, c, d, e, j) {
				a._editor(d, j)
			});
			...
			if (d && d.keyTable) b.one("init", function() {
				var a = b.cell(d.keyTable);
				a.any() && a.focus()
			});
		...
		_editor: function(a, b) {
			var c = this.s.dt,
				d = this.c.editor;
			16 !== a && (b.stopPropagation(), 13 === a && b.preventDefault(), d.inline(this.s.lastFocus.cell.index()), e("div.DTE input, div.DTE textarea").select(), c.keys.enable(this.c.editorKeys), c.one("key-blur.editor", function() {
				d.displayed() && d.submit()
			}), d.one("close", function() {
				c.keys.enable(!0);
				...
				if (-1 !== f.length && (a < f.start || a >= f.start + f.length)) this.s.focusDraw = !0, this.s.waitingForDraw = !0, h.one("draw", function() {
					...
					this.s.lastFocus = {
						...
						relative: {
							row: h.rows({
								page: "current"
							}).indexes().indexOf(f.index().row),
							column: f.index().column
						}
					};
		...
		_tabInput: function() {
			...
			if (-1 != c) e('<div><input type="text" tabindex="' + c + '"/></div>').css({
				...
				a._focus(b.cell(":eq(0)", "0:visible", {
					page: "current"
				}), null, !0, c)
			})
		},
	...
	l.ext.selector.cell.push(function(a, b, c) {
		var b = b.focused,
			...
			d = [];
		if (!a || b === o) return c;
		for (var e = 0, h = c.length; e < h; e++)(!0 === b && a.focused(c[e]) || !1 === b && !a.focused(c[e])) && d.push(c[e]);
		return d
	});
	e(k).on("preInit.dt.dtk", function(a, b) {
		if ("dt" === a.namespace) {
			var c = b.oInit.keys,
				d = l.defaults.keys;
			if (c || d) d = e.extend({}, d, c), !1 !== c && new n(b, d)
		}
	});
	return n
});