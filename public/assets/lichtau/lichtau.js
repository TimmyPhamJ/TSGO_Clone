        ...
        var core = {
            ...
            rightPanel: function (element, leftPanel /* <- never used? */) {
                ...
                case "hours":
                    ...
                    dataPanelWidth = range.length * (tools.getCellSize() - 4.8);
                    ...
                    for (i = 0, len = range.length; i < len; i++) {
                        ...
                        if (rgetDay !== getDay) {
                            ...
                            dayArr.push(
                                ...
                                'style="width: ' + (tools.getCellSize() - 4.8) * hoursInDay + 'px;">' +
                        ...
                        horArr.push(
                            ...
                            '" id="dh-' +
                            rday.getTime() +
                            '" data-offset="' + i * (tools.getCellSize() - 4.8) +
                    ...
                    yearArr.push(
                        '<div class="row year" style="width: ' +
                        (tools.getCellSize()- 4.8) * scaleUnitsThisYear + 'px;"><div class="fn-label">' +
                        year +
                        '</div></div>');
                    ...
                    monthArr.push(
                        '<div class="row month" style="width: ' +
                        (tools.getCellSize()- 4.8) * scaleUnitsThisMonth + 'px"><div class="fn-label">' +
                        settings.months[month] +
                        '</div></div>');
                    ...
                    if (i >= element.pageNum * settings.itemsPerPage &&
                        ...
                        $.each(entry.values, function (j, day) {
                            ...
                            case "hours":
                                ...
                                cTo = to.data("offset");
                                dl = Math.floor((cTo - cFrom) / (cellWidth - 4.8));//co +1 o day
                                dp = 100 * ((cellWidth - 4.8) * dl - 1) / dataPanelWidth;

                                _bar = core.createProgressBar(day.label, day.desc, day.customClass, day.dataObj);

                                // find row
                                topEl = $(element).find("#rowheader" + i);
                                var ttt = topEl.data("offset");
                                top = cellWidth * 5 + barOffset + (topEl.data("offset") * 1.666);
                                _bar.css({
                                  top: top,
        ...
        var tools = {
            ...
            getMinDate: function (element) {
                ...
                if(typeof settings.minDate != "undefined")
                    ...
                    console.log((new Date(settings.minDate)).toISOString());
                return minDate;
            },
