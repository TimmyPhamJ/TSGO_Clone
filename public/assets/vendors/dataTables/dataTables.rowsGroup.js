...
RowsGroup.prototype = {
	...
	{
		if( this.columnsForGrouping.filter( p => Array.isArray(p)).length > 0 ){
			this.columnsForGrouping.forEach(element => {
				var columnsIndexes = this.table.columns(element, ShowedDataSelectorModifier).indexes().toArray()
