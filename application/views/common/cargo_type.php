				...
				<div class="col-md-12 col-sm-12 col-xs-12 table-responsive">
					<table id="contenttable" class="table table-striped display nowrap" cellspacing="0" style="width: 99.5%">
						...
						<tr>
							...
							<th col-name="CargoTypeID">Mã loại hàng</th>
							<th col-name="CargoTypeName">Tên loại hàng</th>
						</tr>
						</thead>
						<tbody>
							<?php if(count($cargoTypeList) > 0) {
								  		$i = 1; ?>
								<?php foreach($cargoTypeList as $item){  ?>									
									<tr>
										<td col-name="STT"><?=$i;?></td>
										<td col-name="rowguid"><?=$item['rowguid'];?></td>
										<td col-name="CargoTypeID"><?=$item['CargoTypeID'];?></td>
										<td col-name="CargoTypeName"><?=$item['CargoTypeName'];?></td>	
									</tr>
									<?php $i++; }  ?>
							<?php } ?>			
						</tbody>
...
<script type="text/javascript">
	$(document).ready(function () {
		var tbl 			= $('#contenttable'),
			_columns 		= ['STT', 'rowguid', 'CargoTypeID', 'CargoTypeName'],
			cargoTypeList 	= {},
			parentMenuList 	= {};
		...
		tbl.newDataTable({
			...
			columnDefs: [
				...
				{ className: "text-center", targets: _columns.getIndexs(['CargoTypeID', 'CargoTypeName'])},
		...
		$('#save').on('click', function(){
            if(tbl.DataTable().rows( '.addnew, .editing' ).data().toArray().length == 0){
				...
				for (i = 0; i < newData.length; i++){
					if (!newData[i]['CargoTypeID']){
						toastr['error']("Vui lòng nhập Mã loại hàng!");
						return;
					}
					else{
						for (j = 0; j < cargoTypeList.length; j++){
							if (newData[i]['CargoTypeID'] == cargoTypeList[j]['CargoTypeID']){
								toastr['error']("Đã tồn tại Mã loại hàng: " + newData[i]['CargoTypeID']);
								return;
							}
						}	
					}		

					if (!newData[i]['CargoTypeName']){
						toastr['error']("Vui lòng nhập Tên cho Mã loại hàng: " + newData[i]['CargoTypeID'] + "!");
						return;
					}
				}
		...
		function postSave(formData){
			...
			$.ajax({
	            url: "<?=site_url(md5('Common') . '/' . md5('cmCargoType'));?>",
	            dataType: 'json',
	            data: formData,
	            type: 'POST',
		...
		function postDel(data){
			...
			var delData = data.map(p=>p[_columns.indexOf("CargoTypeID")]);
	            ...
	            success: function (data) {
		            ...
		            if(data.success && data.success.length > 0){
		            	for (var i = 0; i < data.success.length; i++) {
		            		...
		            		var indexes = tbl.filterRowIndexes( _columns.indexOf( "CargoTypeID" ), valueID);
		            		tbl.DataTable().rows( indexes ).remove().draw( false );
		            		tbl.updateSTT( _columns.indexOf( "STT" ) );
		            		toastr["success"]( data.success[i] );

		            		for (j = 0; j < cargoTypeList.length; j++){
		            			if (cargoTypeList[j]['CargoTypeID'] == valueID){
		            				cargoTypeList.splice(j, 1);
		            			}
		            		}
		            	}
		            }
