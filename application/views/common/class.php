...
<div class="row">
	<div class="col-xl-12">
		<div class="ibox collapsible-box">
			...
			<div class="ibox-head">
				<div class="ibox-title">HƯỚNG NHẬP XUẤT</div>
				...
				<div class="col-md-12 col-sm-12 col-xs-12 table-responsive">
					<table id="contenttable" class="table table-striped display nowrap" cellspacing="0" style="width: 99.5%">
						...
						<tr>
							...
							<th col-name="ClassID">Mã hướng nhập xuất</th>
							<th col-name="ClasName">Mô tả</th>
						</tr>
						</thead>
						<tbody>
							<?php if(count($classList) > 0) {
								...
								<?php foreach($classList as $item){  ?>									
									<tr>
										...
										<td col-name="ClassName"><?=$item['ClassName'];?></td>		
									</tr>
									<?php $i++; }  ?>
							<?php } ?>	
						</tbody>
...
<script type="text/javascript">
	$(document).ready(function () {
		var tbl 			= $('#contenttable'),
			_columns 		= ['STT', 'rowguid', 'ClassID', 'ClassName'],
			classList		= {};
		...
		var dataTbl = tbl.newDataTable({
			...
			columnDefs: [
				...
				{ className: "text-center", targets: _columns.getIndexs(['ClassID', 'ClassName'])},
            ...
            if(tbl.DataTable().rows( '.addnew, .editing' ).data().toArray().length == 0){
				...
				for (i = 0; i < newData.length; i++){
					if (!newData[i]['ClassID']){
						toastr['error']("Vui lòng nhập Mã hướng nhập xuất");
						return;
					}
					else{
						for (j = 0; j < classList.length; j++){
							if (newData[i]['ClassID'] == classList[j]['ClassID']){
								toastr['error']("Đã tồn tại Mã hướng: " + newData[i]['ClassID']);
								return;
							}
						}	
					}		

					if (!newData[i]['ClassName']){
						toastr['error']("Vui lòng nhập Mô tả cho Mã hướng nhập xuất: " + newData[i]['ClassID'] + "!");
						return;
					}
				}
		...
		function postSave(formData){
			...
			$.ajax({
                url: "<?=site_url(md5('Common') . '/' . md5('cmClass'));?>",
                dataType: 'json',
                data: formData,
                type: 'POST',
		                ...
		                if(data.error && data.error.length > 0){
		                	for (var i = 0; i < data.error.length; i++) {
				                ...
				                var	str 	= data.error[i].split(':')[1].trim(),
				                	...
				                	cell 	= tbl.find("tbody tr:eq(" + cRow + ") td:eq("+ _columns.indexOf("ClassID") +")");			    
		...
		function postDel(data){
			...
			var delData = data.map(p=>p[_columns.indexOf("ClassID")]);
                ...
                success: function (data) {
	                ...
	                if(data.success && data.success.length > 0){
	                	for (var i = 0; i < data.success.length; i++) {
	                		...
	                		var indexes = tbl.filterRowIndexes( _columns.indexOf( "ClassID" ), valueID);
