...
router.get('/', auth, async function (req, res, next) {
    let dataList=await CommonModel.loadService(req);
    let JobModeList=await CommonModel.loadJobMode(req);
    res.loadContent('common/cmServices', {dataList,JobModeList});
});

router.post('/get', auth, function (req, res, next) {
    CommonModel.loadService(req).then((data)=>{
...
router.post('/save', auth, async function (req, res, next) {
    CommonModel.saveService(req).then((data)=>{
        res.status(200).json({data});
    }).catch((error)=>{
        res.status(200).json({error});
    });
...
router.post('/delete', auth, async function (req, res, next) {
    CommonModel.deleteService(req).then((data)=>{
        res.status(200).json({data});
    }).catch((error)=>{
        res.status(200).json({error});
    });
