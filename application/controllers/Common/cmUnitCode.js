...
router.get('/', auth, async function (req, res, next) {
    let dataList=await CommonModel.loadUnit(req);
    res.loadContent('common/cmUnitCode', {dataList});
});

router.post('/get', auth, function (req, res, next) {
    CommonModel.loadUnit(req).then((data)=>{
...
router.post('/save', auth, async function (req, res, next) {
    CommonModel.saveUnit(req).then((data)=>{
        res.status(200).json({data});
    }).catch((error)=>{
        res.status(200).json({error});
    });
...
router.post('/delete', auth, async function (req, res, next) {
    CommonModel.deleteUnit(req).then((data)=>{
        res.status(200).json({data});
    }).catch((error)=>{
        res.status(200).json({error});
    });
