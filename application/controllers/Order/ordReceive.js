...
router.get("/", auth, async (req, res, next) => {
  ...
  let items =
    (await CommonModel.loadItem(req).catch((err) => console.log(err))) || [];
  let units =
    (await CommonModel.loadUnit(req).catch((err) => console.log(err))) || [];
  let transits =
    (await CommonModel.loadTransit(req).catch((err) => console.log(err))) || [];
