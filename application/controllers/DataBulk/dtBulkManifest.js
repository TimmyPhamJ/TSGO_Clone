...
router.get('/', auth, async function (req, res, next) {
    ...
    res.loadContent('DataBulk/dtBulkManifest', {dataList,jobModeList,classList,methodList,unitList,itemList,customerList,customerTypeList,transitList});
});

router.post('/getCusSearch', auth, function (req, res, next) {
    BulkModel.getCusSearch(req).then((data)=>{
...
router.post('/getLaneSearch', auth, function (req, res, next) {
    BulkModel.getLaneSearch(req).then((data)=>{
        res.status(200).json({data});
    }).catch((error)=>{
        console.log(error);
        res.status(200).json({error});
