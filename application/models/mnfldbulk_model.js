...
module.exports.LoadHaHang = async (req) => {
  let query = req
    .gtos("DT_MNF_LD_BULK")
    .select(
      "DT_MNF_LD_BULK.*",
    ...
    .leftJoin("DT_BLOCK_STOCK", function () {
      this.on(
        req.gtos.raw(
          "CONCAT(DT_MNF_LD_BULK.BillOfLading,DT_MNF_LD_BULK.BookingNo)"
        ),
        "=",
        req.gtos.raw(
          "CONCAT(DT_BLOCK_STOCK.BillOfLading,DT_BLOCK_STOCK.BookingNo)"
        )
      )
        .andOn("DT_MNF_LD_BULK.ClassID", "=", "DT_BLOCK_STOCK.ClassID")
        .andOn("DT_MNF_LD_BULK.VoyageKey", "=", "DT_BLOCK_STOCK.VoyageKey");
    })
    .leftJoin("BS_UNIT", "BS_UNIT.UnitID", "DT_MNF_LD_BULK.UnitID")
    .leftJoin(
      "BS_CUSTOMER_TYPE",
...
module.exports.LoadGiaoHang = async (req) => {
  let query = req
    .gtos("DT_MNF_LD_BULK")
    .select(
      "DT_MNF_LD_BULK.*",
